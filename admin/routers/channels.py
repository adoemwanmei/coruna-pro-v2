...
def _verify_sign(payload_body: bytes, api_key: str, body: S2SRegisterRequest) -> bool:
    if not body.sign or not body.ts or not body.nonce:
        return False
    try:
        if abs(int(body.ts) - int(datetime.now().timestamp())) > 300:
            return False
    except Exception:
        return False
    body_dict = body.dict()
...
async def s2s_register(request: Request, channel_id: int, body: S2SRegisterRequest, db: Session = Depends(get_db)):
    ...
    if not _verify_sign(await request.body(), c.api_key or "", body):
